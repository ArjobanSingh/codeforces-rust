...
mod trees {
    ...
    impl SegmentTree {
        ...
        fn get_query_inner(
            ...
            if let Some(ans) = self.get_query_inner(node * 2, l, mid, ql, qr) {
                return Some(ans);
            }
            return self.get_query_inner(node * 2 + 1, mid + 1, r, ql, qr);
        }

        pub fn get_query(&mut self, ql: usize, qr: usize) -> Option<i32> {
            self.get_query_inner(1, 1, self.n, ql, qr)
        }
    }
}
...
fn main() {
    ...
    (1..=n).for_each(|i| {
        write!(out, "{:?} ", st.get_query(i, i).unwrap()).expect("Error writing answer");
    });
